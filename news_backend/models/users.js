...
Users.add = (data, callback) => {
    pool.getConnection((err, connection) => {
        ...
        var query = 'INSERT INTO users (name, password, email, register_time,user_id) VALUES (?, ?, ?, ?,?)';
        var register_time = new Date().getTime();
        connection.query(query, [data.name, data.password, data.email, register_time, data.user_id], (err, results, fields) => {
            if (err) {
...
Users.getOneByPassword = (data, callback) => {
    pool.getConnection((err, connection) => {
        ...
        var query = 'SELECT name,user_id FROM users WHERE (name = ? OR email = ?) AND password = ?';
        connection.query(query, [data.name, data.name , data.password,data], (err, results, fields) => {
            if (err) {
                console.log("Select Error: " + err);
            }else{
                var {login_id,user_id,visit_id,login_time} = data;
                var query1 = 'INSERT INTO login(login_id,user_id,visit_id,login_time) VALUES (?,?,?,?)';
              
                connection.query(query1, [login_id,user_id,visit_id,login_time], (err, results1, fields) => { 
                    if (err) {
                        console.log("Select Error: " + err);
                    }else{
                        if(results.affectedRows !== 0){
